...
@endsection
@section('content')
    @include('shop::admin.breadcrumbs')
        ...
        <div class="bg-grey top-search-bar">
            ...
            <div class="action-mass-buttons pull-right">
                @can(\$MODULE_NAMESPACE$\$MODULE$\$NAME$::PREVIEW_AND_EDIT_PERMISSION)
                    <a href="{{ route('shop.create') }}" role="button" class="btn btn-lg tooltips green" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="@lang('admin.common.create_new')">
                ...
                <div class="hidden" id="mass-delete-url">{{ route('shop.delete-multiple') }}</div>
                @endcan
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-xs-12">
        <h3>@lang('shop::admin.'shop.index'): @lang('adboxes::'shop.ad_boxes_type_1')</h3>
        <div class="table-responsive">
            <table class="table">
                <thead>
                <th class="width-2-percent"></th>
                <th class="width-2-percent">@lang('shop::admin.number')</th>
                <th class="width-130">@lang('shop::admin.type')</th>
                <th>@lang('shop::admin.title')</th>
                <th class="width-220 text-right">@lang('shop::admin.actions')</th>
                </thead>
                <tbody>
                <?php $i = 1;?>
                @forelse ($shopAdmin as $$CLASS$)
                    <tr class="t-row row-{{$$CLASS$->id}}" data-toggle="popover" data-content='<img class="thumbnail img-responsive" src="{{ $$CLASS$->getFileUrl() }}"/>'>
                        ...
                        <td><label class="label btn-light-green">@lang('shop::admin.ad_boxes_type_1')</label></td>
                        <td>{{ $$CLASS$->title }}</td>
                        <td class="pull-right">
                        @can(\$MODULE_NAMESPACE$\$MODULE$\$NAME$::PREVIEW_AND_EDIT_PERMISSION)
                            ...
                            @endif
                            @if($i !== 1)
                                <a href="{{ route('shop.position-up', ['id' => $$CLASS$->id]) }}" role="button" class="move-up btn yellow" data-placement="auto" title="" data-original-title="@lang('admin.common.move_up')"><i class="fas fa-angle-up"></i></a>
                            @endif
                            @if($i != count($shopAdmin))
                                <a href="{{ route('shop.position-down', ['id' => $$CLASS$->id]) }}" role="button" class="move-down btn yellow" data-placement="auto" title="" data-original-title="@lang('admin.common.move_down')"><i class="fas fa-angle-down"></i></a>
                            @endif
                            <a href="{{ route('shop.delete', ['id' => $$CLASS$->id]) }}" class="btn red" data-toggle="confirmation"><i class="fas fa-trash-alt"></i></a>
                        @endcan
                        </td>
                    </tr>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="no-table-rows">@lang('shop::admin.no_records_found')</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
