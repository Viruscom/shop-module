    ...
    <div class="col-xs-12 p-0">
        <div class="bg-grey top-search-bar">
            ...
            <div class="action-mass-buttons pull-right">
                <a href="{{ route('products.stocks.internal_suppliers.create') }}" role="button" class="btn btn-lg tooltips green" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Създай нов">
                    ...
                    @if(count($suppliers))
                        ...
                        @foreach($suppliers as $supplier)
                            ...
                            <tr class="t-row row-{{$supplier->id}}">
                                ...
                                <td class="pull-right">
                                    <a href="{{ route('products.stocks.internal_suppliers.edit', ['id' => $supplier->id]) }}" class="btn green" role="button"><i class="fas fa-pencil-alt"></i></a>
                                    {{--                                    @if(!$supplier->active)--}}
                                    ...
                                    {{--                                    @endif--}}
                                    @if($i !== 1)
                                        <a href="{{ url('/admin/shop/suppliers/move/up/'.$supplier->id) }}" role="button" class="move-up btn yellow"><i class="fas fa-angle-up"></i></a>
                                    @endif
                                    @if($i != count($suppliers))
                                        <a href="{{ url('/admin/shop/suppliers/move/down/'.$supplier->id) }}" role="button" class="move-down btn yellow"><i class="fas fa-angle-down"></i></a>
                                    @endif
                                    <a href="{{ route('products.stocks.internal_suppliers.change-archive-status', ['id' => $supplier->id, 'archived' => ($supplier->archived) ? 0: 1]) }}" role="button" class="btn btn-light-blue m-b-0" data-toggle="confirmation" data-placement="auto" title="" data-original-title="Архивирай"><i class="fas fa-archive"></i></a>
                                </td>
                            </tr>
                            <?php $i++;?>
                        @endforeach
