...
@section('content')
    <div class="col-xs-12 p-0">
        <form class="my-form" action="{{ url('/admin/shop/collections/'.$collection->id.'/update') }}" method="POST" data-form-type="store" enctype="multipart/form-data">
                    ...
                    <div class="col-md-4 col-xs-6">
                        <select name="main_product_id" id="main_product_select" class="form-control select2">
                            @foreach($products as $product)
                                @php
                                    $productTranslation = $product->translations()->where('language_id', 1)->first()
                                @endphp
                                <option value="{{ $product->id }}" {{($collection->main_product_id==$product->id || ($product->id==old('main_product_id'))) ? 'selected': ''}}>{{ $productTranslation->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group @if($errors->has('main_product_id')) has-error @endif">
                        ...
                        <div class="col-md-4 col-xs-4">
                            <select name="additionalProducts[{{$i}}][product_id]" class="form-control select2">
                                @if($i)
                                    <option value="">--- Моля, изберете ---</option>@endif
                                @foreach($products as $product)
                                    @php
                                        $productTranslation = $product->translations()->where('language_id', 1)->first()
                                    @endphp
                                    <option value="{{ $product->id }}" {{ ($product->id == $collectionProduct->additional_product_id) ? 'selected' : ''}}>{{ $productTranslation->title }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('main_product_id'))
                                <span class="help-block">{{ trans($errors->first('main_product_id')) }}</span>
                                ...
                                <select name="additionalProducts[{{$i}}][product_id]" id="" class="form-control select2">
                                    ...
                                    @foreach($products as $product)
                                        @php
                                            $productTranslation = $product->translations()->where('language_id', 1)->first()
                                        @endphp
                                        <option value="{{ $product->id }}">{{ $productTranslation->title }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('main_product_id'))
                                    <span class="help-block">{{ trans($errors->first('main_product_id')) }}</span>
