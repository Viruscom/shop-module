...
@section('content')
    <div class="col-xs-12 p-0">
        <form class="my-form" action="{{ url('/admin/product_categories/'.$productCategory->id.'/update') }}" method="POST" enctype="multipart/form-data" data-form-type="update">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="position" value="{{(old('position')) ? old('position') : $productCategory->position}}">
            <div class="navigation-id-old hidden">{{old('navigation_id')}}</div>
            <div class="navigation-id-current hidden">{{$productCategory->navigation_id}}</div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="alert alert-warning"><strong>Внимание! </strong>Промяната на загланието (името) или на активността (видимостта) на продуктовата категория ще се отрази в sitemap-a на сайта и може да доведе до промени в индексирането на Вашия сайт от търсачките.</div>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-12 col-xs-12">
            ...
            <div class="tab-content">
                @foreach($languages as $language)
                    ...
                    $productCategoryTranslation = (is_null($productCategory->translations->where('language_id', $language->id)->first())) ? null : $productCategory->translations->where('language_id', $language->id)->first();
                    ?>
                    <div id="{{$language->code}}" class="tab-pane fade in @if($language->code == env('DEF_LANG_CODE')) active @endif}}">
                        <div class="form-group @if($errors->has($langTitle)) has-error @endif">
                            ...
                            <input class="form-control" type="text" name="{{$langTitle}}" value="{{ old($langTitle) ? old($langTitle) : (!is_null($productCategoryTranslation) ? $productCategoryTranslation->title : '') }}">
                        ...
                        <div class="form-group @if($errors->has($langShortDescr)) has-error @endif">
                            ...
                            <textarea name="{{$langShortDescr}}" class="form-control" rows="3">{{ (old($langShortDescr)) ? old($langShortDescr) : (!is_null($productCategoryTranslation) ? $productCategoryTranslation->short_description : '') }}</textarea>
                ...
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            ...
                            <div>
                                @if (!is_null($productCategory->filename) && file_exists($productCategory->fullImageFilePath()))
                                    <div class="overlay-delete-img hidden">
                                        <a href="{{ url('/admin/product_categories/'.$productCategory->id.'/img/delete') }}" class="del-link"><i class="fas fa-times"></i>
                                            <p>Изтрий</p></a>
                                    </div>
                                    <img class="thumbnail content-box1 has-img img-responsive" src="{{ $productCategory->fullImageFilePathUrl() }}" width="300"/>
                                @else
                                    <img class="thumbnail img-responsive" src="{{ asset('/admin/assets/system_images/product_category_img.png') }}" width="300"/>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <label class="switch pull-left">
                                <input type="checkbox" name="active" class="success" data-size="small" {{(old('active') ? 'checked' : (($productCategory->active) ? 'checked': ''))}}>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <p class="position-label">№ {{ $productCategory->position }}</p>
                    ...
                    <div class="modal-body">
                        <table class="table table-striped table-hover table-positions">
                            ...
                            @if(count($productCategories))
                                @foreach($productCategories as $productCateg)
                                    <tr class="pickPositionTr" data-position="{{$productCateg->position}}">
                                        <td>{{$productCateg->position}}</td>
                                        <td>{{$productCateg->translations->firstWhere('language_id',$defaultLanguage->id)->title}}</td>
                                    </tr>
                                @endforeach
                        ...
                        <div class="form-actions">
                            <div class="row">
                                <div class="col-md-offset-3 col-md-9">
                                    ...
                                    <a role="button" class="btn back-btn margin-bottom-10 cancel-position-change" current-position="{{ $productCategory->position }}" data-dismiss="modal"><i class="fa fa-reply"></i> назад</a>
                                </div>
                            </div>
                        </div>
                    </div>
